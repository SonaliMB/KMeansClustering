...
if uploaded_file:
    ...
    with col1:
        ...
        st.bar_chart(df["Cluster"].value_counts(),x_label="Cluster", y_label="Count")
    ...
    sns.scatterplot(
        ...
        palette="Set2", s=30, ax=ax
    )
    centers = scaler.inverse_transform(model.cluster_centers_)
    ax.scatter(centers[:, 0], centers[:, 1], c="red", s=70, marker="X", label="Centers")
    ax.legend()
    ...
    if st.button("Predict Cluster"):
        ...
        sns.scatterplot(
            x=df["Age"], y=df["Income"], hue=df["Cluster"],
            palette="Set2", s=30, ax=ax2
        )
        ax2.scatter(age, income, c="black", s=70, marker="*", label="New Point")
        ax2.legend()
